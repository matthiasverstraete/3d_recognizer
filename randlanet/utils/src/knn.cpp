...
pair<at::Tensor, at::Tensor> _single_batch_knn(
    at::Tensor support, at::Tensor query, int k) {
    CHECK_CONTIGUOUS(support);
    CHECK_CONTIGUOUS(query);
    if (support.size(0) < k)
        TORCH_CHECK(false,
                    "Not enough points in support to find " + std::to_string(k) + " neighboors")
    vector<int64_t> neighbors_indices(query.size(0) * k, -1);
    vector<float> neighbors_dists(query.size(0) * k, -1);
    ...
    AT_DISPATCH_ALL_TYPES(query.scalar_type(), "knn", [&] {
        ...
        vector<scalar_t> queries_stl =
            vector<scalar_t>(data_q, data_q + query.size(0) * query.size(1));
        vector<scalar_t> supports_stl =
            vector<scalar_t>(data_s, data_s + support.size(0) * support.size(1));
    ...
    return make_pair(out.clone(), out_dists.clone());
}

pair<at::Tensor, at::Tensor> knn(
    at::Tensor support, at::Tensor query, int k) {
    CHECK_CONTIGUOUS(support);
    CHECK_CONTIGUOUS(query);
    CHECK_CPU(query);
    ...
    return make_pair(out_idx, out_dist);
}
